-- GAP found by walking the real signup flow: the seed chart had Drawings and
-- Reserves & Surplus but NO capital account, so a brand-new company could not
-- record the single most likely first transaction — an investor putting money in.
-- Per-investor accounts (Capital - Anand, etc.) are created when investors are
-- added; this is the generic account that makes the books usable on day one.
create or replace function public.seed_capital_account(p_company uuid)
returns void language sql security definer set search_path = public as $$
  insert into accounts (company_id, code, name, account_type, account_group,
                        sub_group, normal_balance, capex_role)
  values (p_company, '3010', 'Partners / Shareholders Capital', 'equity',
          'Owners Funds', 'Partners Capital', 'C', 'capital')
  on conflict (company_id, code) do nothing;
$$;

revoke all on function public.seed_capital_account(uuid) from public, anon, authenticated;

-- fold it into company creation
...
-- backfill companies that already exist
do $$
declare c record;
begin
  for c in select id from companies loop
    perform public.seed_capital_account(c.id);
  end loop;
end $$;
