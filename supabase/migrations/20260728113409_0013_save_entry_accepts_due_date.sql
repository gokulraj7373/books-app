-- save_journal_entry now carries due_date, so a credit bill records when it is
-- payable at the moment it is posted rather than being patched afterwards.
-- (Patching would also have worked, because due_date is not in the immutability
-- trigger's protected column list — payment terms can legitimately be
-- renegotiated — but setting it at post time keeps one write path.)
...
begin
  ...
  insert into journal_entries (
    ...
    adjustment_reason, source, idempotency_key, due_date, created_by)
  values (
