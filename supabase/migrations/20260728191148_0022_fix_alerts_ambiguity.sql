-- FIX: the OUT parameter named `id` shadowed books.id and companies.id, so the
-- function raised "column reference id is ambiguous" on its first query. Every
-- table reference is now aliased and qualified.
...
begin
  ...
  select count(*) into v_noproof from journal_entries je
   where je.company_id = p_company and je.status = 'posted'
     and coalesce(je.proof_url,'') = '' and je.voucher_type in ('payment','purchase');
  if v_noproof > 0 then
