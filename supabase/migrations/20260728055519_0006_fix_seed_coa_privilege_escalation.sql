-- SECURITY FIX. 0005 granted seed_chart_of_accounts(uuid) to `authenticated`.
-- Because it is SECURITY DEFINER, any signed-in user could POST a guessed
-- company_id to /rest/v1/rpc/seed_chart_of_accounts and inject ~90 accounts
-- into ANOTHER TENANT'S chart of accounts. Cross-tenant write. Not theoretical.
--
-- It never needed that grant: create_company is itself SECURITY DEFINER and can
-- call it regardless of what the caller may execute.
--
-- Two layers, because one is not enough:
--   1. revoke the grant  (removes the endpoint)
--   2. assert membership inside the function (survives a future re-grant)

create or replace function public.seed_chart_of_accounts(p_company uuid)
returns int language plpgsql security definer set search_path = public as $$
declare n int;
begin
  -- auth.uid() is null when called by a trigger/service context; a signed-in
  -- caller must belong to the owning organization.
