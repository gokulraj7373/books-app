-- company_alerts computes "does the trial balance tally", "which bills are
-- overdue" etc from account_balances/open_bills/capex_summary — all of which
-- just gained a p_solo switch. Alerts must use the SAME scope as whatever the
-- owner is currently looking at, or the notification panel would say one
-- thing while the trial balance right next to it says another — the precise
-- bug this whole session has been about closing.
create or replace function public.company_alerts(p_company uuid, p_book uuid, p_solo boolean default false)
...
begin
  ...
  select coalesce(sum(ab.closing_debit),0), coalesce(sum(ab.closing_credit),0)
    into v_dr, v_cr from public.account_balances(p_company, p_book, null, null, p_solo) ab;
  ...
  -- the audit-trail check always covers the whole official book, regardless
  -- of which scope the owner is currently viewing reports in
  ...
  select coalesce(sum(ab.net),0) into v_cash
    from public.account_balances(p_company, p_book, null, null, p_solo) ab where ab.is_bank_or_cash;
  if v_cash < 0 then
  ...
  for r in
    select count(*) n, coalesce(sum(ob.outstanding),0) amt
      from public.open_bills(p_company, p_book, p_solo) ob
  ...
  select coalesce(sum(uc.amount),0) into v_unapplied
    from public.unapplied_credits(p_company, p_book, p_solo) uc;
  ...
  -- investor tracking spans both books by design (a commitment is fulfilled
  -- from either), so it deliberately ignores p_solo
  ...
  for r in
    select cs.name, cs.budget_amount, cs.cwip_balance
      from public.capex_summary(p_company, p_book, p_solo) cs
...
create or replace function public.live_alerts(p_company uuid, p_book uuid, p_solo boolean default false)
...
as $$
  with all_alerts as (
    select a.id, a.severity, a.title, a.body, a.href, a.amount
      from public.company_alerts(p_company, p_book, p_solo) a
    union all
...
revoke all on function public.company_alerts(uuid, uuid, boolean) from public, anon;
revoke all on function public.live_alerts(uuid, uuid, boolean) from public, anon;
grant execute on function public.live_alerts(uuid, uuid, boolean) to authenticated;
-- company_alerts stays internal: only reachable through live_alerts.
