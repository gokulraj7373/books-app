...
create or replace function public.verify_chain(p_company uuid, p_book uuid)
returns bigint
language plpgsql security definer set search_path = public as $$
declare
  r        record;
...
begin
  ...
  for r in
    select id, seq, voucher_no, entry_date, hash, prev_hash
      from journal_entries
     where company_id = p_company and book_id = p_book and seq is not null
     order by seq
  loop
    select string_agg(l.line_no || ':' || l.account_id || ':' || l.base_debit || ':' || l.base_credit,
                      '|' order by l.line_no)
      into v_canon from journal_lines l where l.entry_id = r.id;

    v_expect := extensions.digest(v_prev ||
                  convert_to(p_company::text || p_book::text || r.voucher_no ||
                             r.entry_date::text || coalesce(v_canon,''), 'UTF8'),
                  'sha256');

    if r.prev_hash is distinct from v_prev or r.hash is distinct from v_expect then
      return r.seq;
    end if;
    v_prev := r.hash;
  end loop;
...
begin
  ...
  update journal_entries set reversed_by_entry_id = v_new, status = 'reversed' where id = p_entry;
  return v_new;
end; $$;

grant execute on function public.save_journal_entry(jsonb) to authenticated;
grant execute on function public.verify_chain(uuid, uuid) to authenticated;
