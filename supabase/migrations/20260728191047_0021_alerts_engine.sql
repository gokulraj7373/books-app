-- ============================================================================
-- 0021 — The alerts engine.
--
-- "Dynamic, like AI but not AI": deterministic rules over the company's own
-- data. Every alert carries an `href` so clicking it lands on the exact screen
-- that fixes it — an alert you cannot act on is just noise.
--
-- Ordered by severity then amount, so the most expensive problem is first.
-- ============================================================================
...
declare
  ...
  v_auth numeric; v_paid numeric; v_name text;
  r record;
begin
  if not public.company_is_member(p_company) then return; end if;

  select id into v_stat from books where company_id = p_company and kind = 'primary';
  select id into v_mgmt from books where company_id = p_company and kind = 'adjustment';
  select authorised_capital, name into v_auth, v_name from companies where id = p_company;

  -- ---------- the books stopped adding up ----------
  select coalesce(sum(closing_debit),0), coalesce(sum(closing_credit),0)
    into v_dr, v_cr from public.account_balances(p_company, p_book);
  ...
  -- ---------- money is negative ----------
  select coalesce(sum(net),0) into v_cash
    from public.account_balances(p_company, p_book) where is_bank_or_cash;
  if v_cash < 0 then
  ...
  -- ---------- overdue bills ----------
  for r in
    select count(*) n, coalesce(sum(outstanding),0) amt
      from public.open_bills(p_company, p_book)
  ...
  -- ---------- payments not linked to a bill ----------
  select coalesce(sum(amount),0) into v_unapplied
    from public.unapplied_credits(p_company, p_book);
  ...
  -- ---------- investor money nobody has classified ----------
  ...
  -- ---------- share capital above the authorised limit ----------
  if coalesce(v_auth,0) > 0 then
    select coalesce(sum(share_capital),0) into v_paid from public.investor_master(p_company);
    if v_paid > v_auth then
      return query select 'authcap'::text, 'warn'::text,
        'Share capital is above your authorised limit'::text,
        (to_char(v_paid,'FM99,99,99,990.00') || ' has been recorded as share capital, but your authorised limit is ' ||
  ...
  -- ---------- an investor is behind on what they committed ----------
  for r in
    select name, still_to_bring, pct_funded
      from public.investor_master(p_company)
     where committed > 0 and pct_funded < 50 and still_to_bring > 0
     order by still_to_bring desc limit 3
  loop
  ...
  -- ---------- spend running ahead of budget ----------
  for r in
    select name, budget_amount, cwip_balance
      from public.capex_summary(p_company, p_book)
     where budget_amount > 0 and cwip_balance > budget_amount
  loop
  ...
  -- ---------- an advance sitting with a supplier for a long time ----------
  for r in
  ...
  -- ---------- entries with no bill attached ----------
  select count(*) into v_noproof from journal_entries
   where company_id = p_company and status = 'posted'
     and coalesce(proof_url,'') = '' and voucher_type in ('payment','purchase');
  if v_noproof > 0 then
    return query select 'noproof'::text, 'info'::text,
      (v_noproof || ' payments have no bill attached')::text,
      'Not urgent, but an investor or your CA will ask for the paperwork behind these.'::text,
      '/entries'::text, null::numeric;
  end if;
end; $$;
