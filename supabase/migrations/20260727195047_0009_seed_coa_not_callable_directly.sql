-- VULNERABILITY (found by the security advisor):
-- seed_chart_of_accounts is SECURITY DEFINER, performs no membership check, and
-- was granted to `authenticated`. Any signed-in user could therefore POST to
-- /rest/v1/rpc/seed_chart_of_accounts with a guessed company_id and inject 90
-- accounts into another tenant's chart of accounts.
--
-- It is only ever needed as an internal step of create_company, which is itself
-- SECURITY DEFINER and so can call it regardless of the caller's grants.
-- Revoking it from `authenticated` removes the endpoint entirely.
--
-- Defence in depth: also assert membership inside the function, so it stays safe
-- if a future migration re-grants it by mistake.
...
-- default_rights is a pure lookup, but pin its search_path anyway so it cannot
-- be influenced by a caller-controlled search_path.
...
grant execute on function public.default_rights(text) to authenticated;
